...
- `-`（削除）ボタンは現状維持（アクティブレイヤー型の全インスタンス削除のまま。
  挙動変更はスコープ外）。
- `+`（追加）コンボの items は「タイムライン全体で未使用の型」から
  「**現在のメイドで未使用の型**」へ変更する。旧選択コンボは型選択時に
  `ChangeActiveLayer(layerType, maidSlotNo)` で現在メイドのインスタンスを自動生成
  できたが、新コンボは既存インスタンスの表示トグル専用でこの導線が消えるため、
  `+` 側へ移して「型は他メイドで使用中だが現在のメイドには無い」ケースの新規作成
  手段を残す（plan-review の指摘による）。
