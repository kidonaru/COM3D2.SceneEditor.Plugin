...
1. **カメラレイヤー**: ボディ非依存で最も安全。SceneEditor の CameraWindow / GameViewManager と接続 — **移植完了（2026-08-23）**。DCM 出力・対象モデル UI は未移植方針に合わせ削除。レイヤー UI（DrawWindow）は**接続しないと決定（2026-08-23 仕様変更）**。レイヤー固有の編集は SE の各ウィンドウ（Camera / ライト / ボーン等）での制御に委ねる（一度 TimelineLayerWindow として実装したが revert 済み）
2. **ライトレイヤー**: SceneEditor の StudioLightManager に接続 — **移植完了（2026-08-23）**。MTE の写真モード Hack 経路は使わず、SE ネイティブ StudioLightManager 接続のアダプタ版を実装。DCM 出力・互換モード実体は未移植方針に合わせ削除
3. **モーションレイヤー**: Phase 0 の検証結果に基づき CRC ボディ対応版 MaidCache 相当を実装。.anm 生成 + ExtendBone / IKHold / FingerBlend / Grounding の毎フレーム適用
4. **表情・指レイヤー**: MaidFaceMorphController / MaidFingerBlendController に接続 — **移植完了（2026-08-23）**。表情は ShapeKey / Eyes の 2 レイヤーで実現（指は MotionTimelineLayer の FingerBlend で移植済み）。接続先は SE コントローラではなく MTE 方式の MaidCache 直接経路（blendshape / 瞳 Transform）。DCM 出力・モデル注視は未移植方針に合わせ削除。CRC ボディと旧ボディ間のシェイプキー名互換（eyeclose1 サフィックス等）と MaidFaceWindow との相互排他は将来課題
...
- **ポーズ編集 → キーフレーム化**: SceneEditor の既存ポーズ編集（IK ドラッグ・ボーン編集）の結果を現在フレームのキーフレームとして登録する経路 — **実装完了（2026-08-23）**。手動登録（登録/全登録ボタン・キーバインド）は移植済みで、加えてドラッグ編集（IK/ボーン回転/指/顔）完了時の自動登録をオプトインで追加（既定 ON。2026-09-08 に操作履歴の確定へ発火元を広げ、履歴対応ウィンドウでのスライダー編集も対象にした。同日にマテリアル編集・シェイプキー編集・サブカメラ・テキスト・BGM 設定・動画も履歴対応し、ライブ演出だけが対象外）
- **Undo/Redo**: キーフレーム操作を HistoryManager のスナップショット方式に統合 — **実装完了（2026-08-23）**。TimelineXml 前後対を IHistoryEntry として SE HistoryManager へブリッジ（AddHistory 単一入口で全タイムライン操作を網羅、Ctrl+Z 一本化）。before は lastCommittedXml で追跡し SE 側 Undo 後の履歴ズレを防止。新規作成・読み込み直後の最初の操作はそれ以前へ戻れないため Undo 対象外
- ~~シーンプリセット連携~~: **スコープ外に決定（2026-08-23 仕様変更）**。タイムラインはシーンプリセットに含めず、MTE フォーマット（TimelineXml）の独立保存のみ対応する
- SelectionManager との連携（Hierarchy で選択したメイド/オブジェクトのトラックへジャンプ等） — **実装完了（2026-08-23）**。選択メイドへのスロット切替と追加ライト選択時のライトレイヤー切替に対応
...
| MTE 本体とのタイムライン XML 互換 | ユーザー資産の移行 | **MTE フォーマットにのみ対応と決定（2026-08-23）**。独自形式は導入せず、version 属性（MTE は CurrentVersion=31）を尊重した MTE 互換 XML を唯一の保存形式とする |
