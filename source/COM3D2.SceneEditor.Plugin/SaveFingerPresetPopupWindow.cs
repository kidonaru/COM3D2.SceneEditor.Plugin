...
{
    /// <summary>
    /// 指プリセット保存時に名前を入力させるモーダル。
    /// SaveFacePresetPopupWindow と同じ IMGUI モーダル方式。
    /// 検証と上書き判定は MaidFingerPresetManager に委ね、実際の保存は呼び出し元へ返す
    /// </summary>
    public class SaveFingerPresetPopupWindow : IGUIWindow
    {
        public static readonly int WINDOW_ID = 8903379;
        ...
        /// <summary>プリセット名の入力値。次回表示時も保持する</summary>
        ...
        private static SaveFingerPresetPopupWindow _instance = null;
        public static SaveFingerPresetPopupWindow instance
            => _instance ?? (_instance = new SaveFingerPresetPopupWindow());
        ...
        public static void Show(Action<string> onSave)
        {
            ...
            window._isOverwrite = MaidFingerPresetManager.Exists(window._presetName);
        }

        private void ConfirmSave()
        {
            _errorMessage = MaidFingerPresetManager.ValidatePresetName(_presetName);
        ...
        private void DrawWindow(int id)
        {
            ...
            _view.DrawLabel("指のプリセット名を入力してください", contentWidth, ROW_HEIGHT);

            _view.DrawTextField(_presetName, contentWidth, ROW_HEIGHT, value =>
            {
                _presetName = value;
                _errorMessage = null;
                _isOverwrite = MaidFingerPresetManager.Exists(value);
            });
