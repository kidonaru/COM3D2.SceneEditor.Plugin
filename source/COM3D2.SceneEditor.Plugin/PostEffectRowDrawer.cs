...
namespace COM3D2.SceneEditor.Plugin
{
    ...
    ///
    /// ポストエフェクトには委譲先の個別ウィンドウが無いため、共有元はレイヤーの
    /// PostEffectTimelineLayer.DrawWindow になる。レイヤー本体は MTE 逐語コピーで
    /// 触らない方針のため、同じ書き込み先 (PostEffectManager のデータ) へ
    /// 同じ意味の編集を行う形でここに用意している。
    /// 値の範囲・既定値はレイヤー UI と同じ TransformData の Info を使う。
