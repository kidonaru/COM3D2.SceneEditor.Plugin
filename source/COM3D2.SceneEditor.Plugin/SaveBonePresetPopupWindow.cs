...
{
    /// <summary>
    /// ボーンプリセット保存時に名前を入力させるモーダル。
    /// SaveFacePresetPopupWindow と同じ IMGUI モーダル方式。
    /// 検証と上書き判定は PartsEditPresetIO に委ね、実際の保存は呼び出し元へ返す
    /// </summary>
    public class SaveBonePresetPopupWindow : IGUIWindow
    {
        ...
        private static SaveBonePresetPopupWindow _instance = null;
        public static SaveBonePresetPopupWindow instance
            => _instance ?? (_instance = new SaveBonePresetPopupWindow());
        ...
        public static void Show(Action<string> onSave)
        {
            ...
            window._isOverwrite = PartsEditPresetIO.Exists(window._presetName);
        }

        private void ConfirmSave()
        {
            _errorMessage = PartsEditPresetIO.ValidatePresetName(_presetName);
        ...
        private void DrawWindow(int id)
        {
            ...
            _view.DrawLabel("プリセット名を入力してください", contentWidth, ROW_HEIGHT);

            _view.DrawTextField(_presetName, contentWidth, ROW_HEIGHT, value =>
            {
                _presetName = value;
                _errorMessage = null;
                _isOverwrite = PartsEditPresetIO.Exists(value);
            });
