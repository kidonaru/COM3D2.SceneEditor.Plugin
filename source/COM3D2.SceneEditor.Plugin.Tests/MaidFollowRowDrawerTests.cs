...
{
    public class MaidFollowRowDrawerTests
    {
        ...
        {
            Assert.Equal(expected, MaidFollowRowDrawer.ToFollowMaidIndex(maidSlotNo, ItemCount));
        }
        ...
        public void ToFollowMaidIndex_選択肢を超えるスロット番号は末尾へ丸める()
        {
            ...
            Assert.Equal(ItemCount - 1, MaidFollowRowDrawer.ToFollowMaidIndex(9, ItemCount));
        }

        [Fact]
        public void ToFollowMaidIndex_選択肢がなしだけでも範囲内に収める()
        {
            Assert.Equal(0, MaidFollowRowDrawer.ToFollowMaidIndex(3, 1));
        }
        ...
        {
            Assert.Equal(expected, MaidFollowRowDrawer.ToFollowMaidSlotNo(index));
        }
    }
}
