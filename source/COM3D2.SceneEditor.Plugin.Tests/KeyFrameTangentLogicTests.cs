...
namespace COM3D2.SceneEditor.Plugin.Tests
{
    public class KeyFrameTangentLogicTests
    {
        ...
        public void ハンドル位置_Out側は始点から右上へ伸びる()
        {
            ...
            var pos = KeyFrameTangentLogic.GetHandlePos(true, 1f, 100f, 0f, Mathf.Sqrt(2f));
        ...
        {
            var pos = KeyFrameTangentLogic.GetHandlePos(true, 0f, 100f, 0f, 10f);
        ...
        {
            var pos = KeyFrameTangentLogic.GetHandlePos(false, 0f, 100f, 0f, 10f);
        ...
        public void マウス位置から正規化タンジェントを求める()
        {
            ...
            Assert.True(KeyFrameTangentLogic.TryGetNormalizedTangent(
                true, new Vector2(50f, 50f), 100f, 0f, out var value));
        ...
        {
            var pos = KeyFrameTangentLogic.GetHandlePos(true, float.NaN, 100f, 0f, 10f);
            Assert.Equal(10f, pos.x, 3);
            Assert.Equal(100f, pos.y, 3);
        }

        [Fact]
        public void 余白ぶん内側に寄った位置から伸びる()
        {
            // 一辺 100・余白 10 なら、始点は (10, 90)。勾配 0 のハンドルは枠から離れる
            var pos = KeyFrameTangentLogic.GetHandlePos(true, 0f, 100f, 10f, 10f);
            Assert.Equal(20f, pos.x, 3);
            Assert.Equal(90f, pos.y, 3);
        }

        [Fact]
        public void 余白ありでも勾配は内側領域を基準に求まる()
        {
            // 内側領域は 80px 四方。始点 (10,90) から右 40・上 40 なら勾配 1
            Assert.True(KeyFrameTangentLogic.TryGetNormalizedTangent(
                true, new Vector2(50f, 50f), 100f, 10f, out var value));
            Assert.Equal(1f, value, 3);
        }
        ...
        public void マウスが原点の真上なら勾配が発散して求まらない()
        {
            ...
            Assert.False(KeyFrameTangentLogic.TryGetNormalizedTangent(
                true, new Vector2(0f, 20f), 100f, 0f, out _));
        }
        ...
        public void マウスが逆側なら求まらない()
        {
            ...
            Assert.False(KeyFrameTangentLogic.TryGetNormalizedTangent(
                true, new Vector2(-5f, 50f), 100f, 0f, out _));
            // In ハンドルは終点より左側だけが有効
            Assert.False(KeyFrameTangentLogic.TryGetNormalizedTangent(
                false, new Vector2(150f, 50f), 100f, 0f, out _));
        }
    }
}
