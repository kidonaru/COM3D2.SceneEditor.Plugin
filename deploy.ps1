...
.DESCRIPTION
    以下を順に実行する。
      ...
      - CHANGELOG.md から Release Note を生成して gh release create でアップロード
#>
param(
...
$pluginName = 'COM3D2.SceneEditor.Plugin'
...
# 該当バージョンの本文（次の見出しの手前まで）を抜き出す
$entryPattern = "(?m)^## \d{4}/\d{2}/\d{2} v$([regex]::Escape($version))[ \t]*\r?$"
$entryMatch = [regex]::Match($changelog, $entryPattern)
if (-not $entryMatch.Success) {
    throw "CHANGELOG.md に $tag のエントリが見つかりませんでした。bump-version.bat を実行しましたか?"
}
$rest = $changelog.Substring($entryMatch.Index + $entryMatch.Length)
$nextMatch = [regex]::Match($rest, '(?m)^#{1,2} ')
$changesRaw = if ($nextMatch.Success) { $rest.Substring(0, $nextMatch.Index) } else { $rest }
$changes = $changesRaw.Trim()
if (-not $changes) {
    throw "CHANGELOG.md の $tag の変更内容が空です"
}
if ($changes -match 'TODO') {
    throw "CHANGELOG.md の $tag の変更内容に TODO が残っています。内容を記載してから実行してください"
}
