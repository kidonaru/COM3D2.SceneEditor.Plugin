...
#### タブバーの挙動（MTEUtils `TabBarDrawer` / `TabBarLayout` 実装）

`TabBarDrawer.Draw(...)` をそのまま使う場合、以下は取り込むだけで有効になる。
自前描画のゲストや旧 MTEUtils のままのゲストは従来挙動（縮小のみ・メニューなし）になる。

- **タブ幅の下限は 60px**。全タブが下限幅で収まらない場合はタブ列の両端に
  `<` `>` ボタンが出る。これは隣のタブをクリックしたのと同じ扱いで、
  前後のタブをアクティブにする（表示位置は下記の寄せが追従する）。
  ボタンの通知先は `ActivateTabIndex` なので、未対応ホストでは効かない。
  送りは**タブ単位ではなく px 単位**で、領域の端に掛かるタブはクリップして
  見切れたまま描く（続きがあることを示すため）。
  描画中はアクティブタブを追従しないので、アクティブタブが隠れた状態もあり得る。
  ただし**タブがアクティブになった時点**だけは、そのタブが見切れていれば
  見える位置まで寄せる（既に収まっていればスクロール位置は動かさない）
- **ヘッダー上でのマウスホイール**でもタブが左右にスクロールする
...
- **起動時のドッキング構成は windowId ベースで復元される**（ゲスト側の対応は不要）。
  ホストは起動時の保存構成を控えておき、ゲストの `Register` 後に元のグループ・並び順・
  アクティブタブへ復帰させる。**この復元は windowId が起動をまたいで安定していることが前提**
  なので、リリース後に windowId を変えないこと。
  保存構成に無い窓は従来どおりヘッダー位置一致で復帰する。
  なお、ユーザーが手動でタブ構成を変えた後は控えた構成を破棄する（古い構成へ巻き戻さない）
