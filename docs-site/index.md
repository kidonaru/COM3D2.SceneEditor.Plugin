...
メイドのポーズ・表情・衣装、カメラ、ライト、背景、サウンドをその場で編集し、撮影まで完結できます。
...
- **メイド操作** — 呼出・配置、IK ドラッグ点によるポーズ付け、モーション（再生・シーク）、表情、指（ブレンド + 関節ごとの個別編集）、視線、IK 固定、脱衣、重力、ボーン編集（装着アイテム / 配置モデル）、シェイプキー、マテリアル
- **カメラ / ライト** — カメラ構図のプリセット（10 スロット）、サブカメラ、メイン/追加ライト（ポイント / スポット / 平行、照らす対象の選択）
- **演出** — 背景切替、ライブ演出（ステージライト / レーザー / サイリウム）、テキスト、動画、サウンド、PNG 配置
- **タイムライン** — キーフレームでモーション・表情・カメラ・ライト・モデル・演出を時間軸で編集。MTE 互換の XML、.anm 出力、連番画像出力
- **シーンプリセット** — メイド・カメラ・背景・ライト・サウンド・PNG 配置をまとめて保存/適用。保存/読込する要素の絞り込み、事務所シーンの自動ロード、SceneCapture プリセットの読み込みにも対応
...
|---|---|---|
| COM3D2.5 | 対応 | 3.49.0 |
| COM3D2 (2.0) | 対応 | 2.49.0 |

推奨バージョンは開発・動作確認に使用しているゲームバージョンです。これより古いバージョンでは動作しない場合があります。
...
- [演出と撮影](/guide/staging) — カメラ・ライブ演出・背景・ライト・サウンド・PNG 配置・スクリーンショット
